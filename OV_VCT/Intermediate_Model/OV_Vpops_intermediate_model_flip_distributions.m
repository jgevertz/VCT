%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%                                                                       %
% FLIP distribution used so accept-or-reject starts with a uniform      %
% distribution and accept-or-perturb starts with a normal distribution. %
% (6/15/2025)                                                           %
%                                                                       %
% This codes generates a virtual population with NVPops virtual         %
% patients for the intermediate OV model:                               %
%   U' = r*U - beta*U*V/N, I' = beta*U*V/N - d_I*I,                     %
%   V' = alpha*d_I*I - d_V*V, N = U + I.                                %
...
%  - NVPops = 500 required generating 3713 plausible patients if we     %
%    truncate the parameters after generating the prior.                %
%    Run time = 17.167396 seconds.                                      %
%                                                                       %
% For Accept-or-Perturb with NVPops = 500 and bounds as specified in    %
% corresponding fit file. Run time = 2390.099535 seconds.               %
%                                                                       %
...
%load best_fit_intermediate.mat 

% Model ICs, fixed parameters, bounds for fit parameters
load param_bounds_intermediate.mat 
% Contains lb, ub, p_fixed = [dI, U0, I0, V0]
...
if VP_method == 1 % accept-or-reject
    % FLIP: Generate plausible patients from a uniform distribution
    ...
    fname_fig_params = 'VPop_distributions_acc_rej_flip';
    fname_fig_trajects = 'VPop_trajectories_acc_rej_flip';
    title_text = 'Accept-or-Reject';
    fsave = 'Vpops_acc_rej_flip.mat';
    
elseif VP_method == 2 % accept-or-perturb
    % FLIP: Generate plausible patients from a normal distribution
    ...
    fname_fig_params = 'VPop_distributions_acc_perturb_flip';
    fname_fig_trajects = 'VPop_trajectories_acc_perturb_flip';
    title_text = 'Accept-or-Perturb';
    fsave = 'Vpops_acc_pert_flip.mat';
else
...
for i = 1:length(lb)
    ...
    elseif i == 2
        histogram(plausible_params(:,i)/1000,'Normalization','pdf'); 
        hold on; 
        histogram(Vpop(:,i)/1000,'Normalization','pdf'); hold off;
...
%                                                 lb,ub,std_params)
%     Nparams = length(mean_params);
%     Ngen = NVpops*100;  % generate more that needed due to rejections
%     plausible_params = zeros(Ngen,Nparams);
%     for i = 1:Nparams 
%         param_mean = mean_params(i); 
%         param_std = std_params(i); 
...
function [Vpop,Traj] = accept_or_reject(NVpops,curve1,curve2,VP_params,...
    ...
    while count_VPs < NVpops
        ...
        fprintf('Plausible patient number %d has r = %f, beta = %f,\n',...
            i,VP_params(i,1), VP_params(i,2)/1000); 
        fprintf('delta_V = %f, alpha = %f\n',VP_params(i,3),1000*VP_params(i,4)); 
...
function [Vpop,Traj] = accept_or_perturb(NVpops,curve1,curve2,VP_params,...
    ...
    while count_VPs < NVpops
        plausible_params = VP_params(i,:);
        fprintf('Plausible patient number %d has r = %f, beta = %f, delta_V = %f, alpha = %f\n',...
            i,VP_params(i,1), VP_params(i,2)/1000, VP_params(i,3), ...
            1000*VP_params(i,4)); 
        VP_attempts = VP_attempts + 1;
        ...
        if accept == 0 % add VP
            ...
            fprintf('\t%d. Perturbing parameters for virtual patient:',i);
