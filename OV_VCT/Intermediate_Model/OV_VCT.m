...
%                                                                       %
% Elapsed time is 137.916043 seconds (for dose_scaling = 1:1:40)        %
% Updated: 6/6/2025                                                     %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load best_fit_intermediate.mat % Contains lb, ub, p_fixed = [dI, U0, I0, V0]
load Vpops_acc_rej.mat  % Contains VP params in order
                        % r, beta, delta_V, alpha
undetectable_threshold = 1; % small
T0 = p_fixed(2);
V0 = p_fixed(4); % experimental dose     
NVPops = size(Vpop,1);
dose_scaling = 1:1:40; 
...
for j = 1:length(dose_scaling)
    p_fixed(4) = V0*dose_scaling(j); % scale dose
...
clear VPop;
load Vpops_acc_pert.mat  % Contains VP params in order
                        % r, beta, delta_V
VPop_acc_pert = Vpop;
...
ax = gca;
ax.FontSize = 14;
saveas(gcf,'VCT_by_method.fig');
exportgraphics(gcf,'VCT_by_method.png');
...
legend('Accept-or-Reject','Accept-or-Perturb','FontSize',16',...
    'location','NorthEast');
saveas(gcf,'VCT_by_classification.fig');
exportgraphics(gcf,'VCT_by_classification.png');

save VCT.mat dose_scaling count_CR_acc_rej count_PR_acc_rej ...
    count_SD_acc_rej count_PD_acc_rej count_CR_acc_pert ...
    count_PR_acc_pert count_SD_acc_pert count_PD_acc_pert ...
    classify_acc_rej classify_acc_pert p_fixed VPop_acc_rej VPop_acc_pert
