...
% patients for the complex OV model:                                    %
...
%  - NVPops = 500 required generating 3719 plausible patients if we     %
%    truncate the parameters after generating the prior.                %
%    Run time = 20.162220 seconds.                                      %
%                                                                       %
% For Accept-or-Perturb with NVPops = 500 and bounds as specified in    %
% corresponding fit file. Run time = 2880.159462 seconds.               %
%                                                                       %
...
% Contains lb, ub, p_fixed = [delta_I, delta_T U0, I0, V0, T0]
% VP params in order = [r, beta, delta_V, alpha, gamma_I, epsilon, lambda]
...
function [Vpop,Traj] = accept_or_reject(NVpops,curve1,curve2,VP_params,...
    ...
    while count_VPs < NVpops
        ...
        fprintf('delta_V = %f, alpha = %f, gamma_I = %f,\n',...
            VP_params(i,3),1000*VP_params(i,4),VP_params(i,5)); 
        fprintf('epsilon = %f, lambda = %f\n',...
            VP_params(i,6),VP_params(i,7)); 
    ...
    while count_VPs < NVpops
        ...
        if accept == 0 % add VP
            ...
            for j = 1:length(traj_sim_ann)
                if (traj_sim_ann(j)<curve2(j))||...
                        (traj_sim_ann(j)>curve1(j)) 
                    accept = -1;
                end
            end 
