...
%dose_contin_vary = linspace(0.01,0.1,totRep);  % dose range
% dose_contin_vary = 0.03;       % dose
firstDose = 15*24+1;           % time first dose is given hours
%%% Set rad on(1) or off(0):
rad=1; %0;
% Directory and files for output
if rad == 1
    dir_name = ['Continuous_Treatment_VaryDose_Radiosens_' num2str(alphaC/betaC)];
else
    dir_name = 'Control';
end
...
for rep=1:totRep
    ...
    fprintf('Simulation has dose = %f, Cm = %f, betaC = %f, and alphaC/betaC = %f\n',...
        dose_contin,Cm,betaC,alphaC/betaC); 
